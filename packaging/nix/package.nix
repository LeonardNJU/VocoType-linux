...
let
  ...
  workers = stdenv.mkDerivation {
    ...
    buildPhase = ''
      ...
      bash src/workers/funasr/build.sh
      runHook postBuild
    '';
...
stdenv.mkDerivation (finalAttrs: {
  ...
  buildPhase = ''
    ...
    ${lib.optionalString withFcitx ''
      cmake -S src/integrations/fcitx5/module -B build-nix/fcitx5 \
        -DCMAKE_BUILD_TYPE=Release \
        -DCMAKE_INSTALL_PREFIX="$out" \
...
WRAPPER
    ...
    install -Dm644 resources/desktop/io.github.LeonardNJU.VoCoType.Settings.desktop \
      "$out/share/applications/io.github.LeonardNJU.VoCoType.Settings.desktop"
    install -Dm644 resources/metainfo/io.github.LeonardNJU.VoCoType.metainfo.xml \
      "$out/share/metainfo/io.github.LeonardNJU.VoCoType.metainfo.xml"
    install -Dm644 web/icon-192.png \
    ...
    install -Dm644 resources/templates/terms.yaml "$out/share/vocotype/terms.yaml"
...
WRAPPER
      ...
      substitute src/integrations/ibus/data/vocotype.xml.in \
    ...
    ${lib.optionalString withFcitx ''
      ...
      install -Dm644 src/integrations/fcitx5/data/vocotype.conf \

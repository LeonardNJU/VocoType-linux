...
usage() {
  cat <<'HELP'
Usage: packaging/scripts/stage-system-package.sh --destdir DIR [options]
...
. "$PROJECT_DIR/packaging/scripts/package-common.sh"
...
install -Dm755 "$PROJECT_DIR/scripts/install/common/install-native-user.sh" \
  "$source_root/scripts/install/common/install-native-user.sh"
...
cmake -S "$PROJECT_DIR/src/core" -B "$CORE_BUILD" \
  -DCMAKE_BUILD_TYPE=Release \
  -DBUILD_TESTING=OFF
...
streaming_bundle=${VOCOTYPE_STREAMING_BUNDLE_DIR:-"$PROJECT_DIR/src/workers/funasr/build/bundle"}
...
if [[ "$INCLUDES_FCITX5" == true ]]; then
  install -Dm755 "$PROJECT_DIR/packaging/common/bin/vocotype-fcitx5-backend" \
    "$DESTDIR$PREFIX/bin/vocotype-fcitx5-backend"
  install -Dm755 "$PROJECT_DIR/packaging/common/bin/vocotype-fcitx5-recorder" \
    "$DESTDIR$PREFIX/bin/vocotype-fcitx5-recorder"
  install -Dm644 "$PROJECT_DIR/src/integrations/fcitx5/data/vocotype.conf" \
    "$DESTDIR$PREFIX/share/fcitx5/addon/vocotype.conf"
  install -Dm644 "$PROJECT_DIR/packaging/common/systemd/vocotype-fcitx5-backend.service" \
  ...
  if [[ "$SKIP_MODULE_BUILD" != true ]]; then
    ...
    cmake -S "$PROJECT_DIR/src/integrations/fcitx5/module" -B "$FCITX_BUILD" \
      -DCMAKE_BUILD_TYPE=Release \
      -DCMAKE_INSTALL_PREFIX="$PREFIX" \
...
if [[ "$INCLUDES_IBUS" == true ]]; then
  ...
  sed -e "s|VOCOTYPE_EXEC_PATH|$LIBEXECDIR/vocotype-ibus-engine|g" \
      ...
      "$PROJECT_DIR/src/integrations/ibus/data/vocotype.xml.in" > \
...
install -Dm644 "$PROJECT_DIR/resources/metainfo/io.github.LeonardNJU.VoCoType.metainfo.xml" \
  "$DESTDIR$PREFIX/share/metainfo/io.github.LeonardNJU.VoCoType.metainfo.xml"
install -Dm644 "$PROJECT_DIR/web/icon-192.png" \

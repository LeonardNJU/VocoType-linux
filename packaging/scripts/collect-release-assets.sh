...
DESTINATION=$(cd "$destination_parent" && pwd)/$(basename "$DESTINATION")
...
while IFS= read -r -d '' path; do
  ...
  if [[ "$INSTALLERS_ONLY" == true ]]; then
    case "$normalized" in
      *.src.rpm) continue ;;
      vocotype-linux*_amd64.deb|vocotype-linux*.x86_64.rpm|vocotype-linux*-x86_64.pkg.tar.zst|VoCoType-linux-*-macOS-arm64.dmg) ;;
      *) continue ;;
    esac
  fi
  [[ ! -e "$DESTINATION/$normalized" ]] || { echo "Duplicate asset: $normalized" >&2; exit 1; }

        ...
        [weak_self, generation](const std::string &type, const std::string &value) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (event_type == "partial") {
              ...
              if (strong_self->_state->recording.load() &&
                  strong_self->_state->last_partial.empty()) {
                const VoiceMode active_mode = strong_self->_state->mode;
                [strong_self showStatus:active_mode == VoiceMode::edit
                    ? @"🎤 正在听编辑指令…"
                    : active_mode == VoiceMode::polish
                        ? @"✨ 正在听，将自动润色…" : @"🎤 正在听…"];
              }
            } else {

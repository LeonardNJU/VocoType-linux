...
`deploy/` 内含 systemd unit、maintenance timer、Nginx 配置、TLS 初始化和运维 CLI。生产服务只监听 `127.0.0.1:18088`，Nginx 负责 HTTPS 与请求大小限制。

部署入口：

```bash
sudo src/services/feedback/deploy/build-install.sh
```

维护任务会在 30 天后删除私有支持包，并将 SQLite 一致性备份写入 `/var/backups/vocotype-feedback`，默认保留 14 天。

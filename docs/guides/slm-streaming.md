...
IBus 与 Fcitx 5 的语音编辑动作（默认 `Ctrl+F9`）都会将 ASR 指令、surrounding text、光标和选区发送给该 API，由模型返回受限的 `replace`、`key_actions` 或 `no_op` JSON 计划。本地执行器只校验并执行计划，不解析自然语言命令。
...
这三个动作都可以在设置中心重新录制；后续文档用默认组合简称对应动作。

...
语音编辑动作默认使用 `Ctrl+F9`，也可以在设置中心重新录制。触发后会先读取当前输入框提供的上下文，再识别编辑指令。所有自然语言理解都交给已启用并保存配置的 SLM：模型结合全文、光标、选区和 ASR 可能产生的同音词错误，返回受限 JSON 编辑计划；IBus 与 Fcitx 5 的本地适配器只负责校验并执行 `replace`、`key_actions` 或 `no_op`。两者的主要差异来自应用向输入法框架暴露上下文的方式。
...
语音编辑因而要求 AI 功能已启用、端点与模型已填写并保存。连接测试仅用于诊断，不是激活状态。配置的 OpenAI-compatible API 会收到 ASR 指令和当前控件提供的 surrounding text、光标与选区；端点可以位于本机或远端。若不希望这些内容离开设备，应自行在本机运行兼容服务，或不要启用语音编辑。

...
V5 Beta 4采用 ad-hoc签名且未经过 Apple公证。先尝试打开一次，然后进入 **系统设置 → 隐私与安全性 → 仍要打开**。ad-hoc没有一张可导入并信任的证书；该操作为具体 App建立本机例外。

## macOS录音过短提示不消失

V5 Beta 4中警告约 2秒后自动关闭。任何时候都可以点击整个状态浮层或按 `Esc`取消并清理；即使录音已经结束、没有活动录音器，点击也会关闭残留提示。

## macOS第一次F9松开后没有上屏

确认 App与输入法组件版本均为 `5.0.0b4`或更高。该版本固定一次操作的文本客户端，延迟按键周期中的全局热键重载，并在麦克风尚未 ready时立即取消。旧版本需要用新 App覆盖 `/Applications`后启动一次，让它自动更新 `~/Library/Input Methods`中的组件。

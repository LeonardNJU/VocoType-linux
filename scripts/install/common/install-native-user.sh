...
SOURCE_MODE=false
if [[ -f "$PROJECT_DIR/src/desktop/CMakeLists.txt" &&
      -f "$PROJECT_DIR/src/workers/funasr/build.sh" ]]; then
  SOURCE_MODE=true
...
install_deps() {
  ...
  if [[ -x "$PROJECT_DIR/scripts/install/common/install-system-dependencies.sh" ]]; then
    if command -v pkexec >/dev/null 2>&1; then
      pkexec bash "$PROJECT_DIR/scripts/install/common/install-system-dependencies.sh" "$flavor"
    else
      sudo bash "$PROJECT_DIR/scripts/install/common/install-system-dependencies.sh" "$flavor"
    fi
  fi
}
...
build_source_runtime() {
  install_deps
  if [[ ! -x "$PROJECT_DIR/src/workers/funasr/build/bundle/bin/vocotype-streaming-worker" ||
        ! -x "$PROJECT_DIR/src/workers/funasr/build/bundle/bin/vocotype-offline-worker" ]]; then
    log "Building pinned FunASR/ONNX native runtime…"
    "$PROJECT_DIR/packaging/scripts/build-native-streaming-release.sh" \
      "$PROJECT_DIR/dist/native" >/tmp/vocotype-native-release-path.txt
  fi
  rm -rf "$STREAMING_HOME"
  mkdir -p "$STREAMING_HOME"
  cp -a "$PROJECT_DIR/src/workers/funasr/build/bundle/." "$STREAMING_HOME/"
...
EOF_SERVICE
  if [[ "$SOURCE_MODE" == true ]]; then
    ...
    cmake -S "$PROJECT_DIR/src/integrations/fcitx5/module" -B "$build" -DCMAKE_BUILD_TYPE=Release
    ...
    install -m644 "$PROJECT_DIR/src/integrations/fcitx5/data/vocotype.conf" \
